...
def wirte_track_mark(num):
    f = open("backend/track_num.conf",'w')
    try:
        f.write(num)
    finally:
        f.close()
...
def user(request):
    ...
    for all_user in all_users:
        ...
        userdict['group'] = group
        bigdict[user] = userdict
...
def adduser(request):
    if request.method == 'GET':
        ...
        user_add = User.objects.create_user(username=username, is_staff='1', password=password)
        user_add.save()
...
def deleteuser(request):
    if request.method == 'GET':
        ...
        all_user = User.objects.filter(username=user_name)
        for group in all_group:
            group_id = group.id
        for user in all_user:
            user_id = user.id
...
def edituser(request, id=None):
    if request.method == 'GET':
        ...
        all_user = User.objects.filter(id=id)
        user = User.objects.get(id=id)
        group = user.groups.get()
...
def macresult(request):
    if request.method =='GET':
        ...
        idc_bh = request.GET['idc_jg']
        try:
            HostList.objects.filter(id=id).update(ip=ip,hostname=name,application=service,idc_name=idc_name,bianhao=idc_bh)
        except:
            print "get exception"
        finally:
            return HttpResponse('ok')
...
def userresult(request):
    if request.method == 'GET':
        ...
        md5pass = md5encryption(password)
        group_id = Group.objects.get(name=groupname).id
        u = User.objects.get(id=id)
        old_g = u.groups.get()
        g = Group.objects.get(id=group_id)
        try:
            User.objects.filter(id=id).update(username=username, password=md5pass)
            u.groups.remove(old_g)
            u.groups.add(g)
        except Exception, e:
            print "Error imformation: %s" % e
        finally:
            return HttpResponse('ok')
...
def getdata(request):
    if request.method == 'GET':
        ...
        if item != 'None':
            ...
            print data_list
            f = open("backend/monitor_data.txt",'w')
            try:
                for i in data_list:
                    f.write(i)
                    f.write("\n")
...
def monitor_data(request):
    data = []
    f = open("backend/monitor_data.txt")
    try:
        lines = f.readlines()
    finally:
        f.close()
